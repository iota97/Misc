...
def add_light(x, y, z, r, g, b, limg):
...
def do_step(s):
	...
	limg = Image.new('RGB', size, (0, 0, 0)).load()
	for i in range(len(position)):
		...
		add_light(x, y, position[i][2], color[i][0], color[i][1], color[i][2], limg)
	if step == 1:
		render(output+".png", limg)
	else:
		render(output+str(s)+".png", limg)
